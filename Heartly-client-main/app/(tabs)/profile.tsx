    ...
    return (
        <SafeAreaView className="flex-1 bg-[#f6f7f8] dark:bg-[#101922]">
            ...
            <View className="flex-row items-center justify-between px-4 py-3">
                <View className="w-8" />
                <Text className="text-lg font-bold text-[#111418] dark:text-white flex-1 text-center">
                    Profile
                </Text>
                <TouchableOpacity
            ...
            >
                ...
                <View className="items-center py-6">
                    <View className="relative">
                        <Image
                            ...
                            className="h-32 w-32 rounded-full border-2 border-white dark:border-slate-800"
                        />
                        <TouchableOpacity
                    ...
                    <Text className="text-[22px] font-bold text-[#111418] dark:text-white mt-4">
                        Alex Johnson
                    </Text>
                    <Text className="text-[#637588] dark:text-[#9dabb9] text-base mt-1">
                ...
                <View className="px-4 mb-4">
                    <Text className="text-lg font-bold text-[#111418] dark:text-white mb-3">At a Glance</Text>

                    {/* Next Appointment Card */}
                    <View className="flex-row bg-white dark:bg-[#1c2127] rounded-2xl p-4 shadow-sm border border-slate-200 dark:border-transparent">
                        <View className="flex-1 justify-center">
                            <View className="flex-row items-center mb-1">
                                <View className="w-2 h-2 rounded-full bg-green-500 mr-2" />
                                <Text className="text-[#637588] dark:text-[#9dabb9] text-[10px] font-bold uppercase">Next Appointment</Text>
                            </View>
                            <Text className="text-[#111418] dark:text-white text-lg font-bold">Dr. Smith (General)</Text>
                ...
                <View className="px-4 mt-8">
                    <Text className="text-lg font-bold text-[#111418] dark:text-white mb-3">Quick Actions</Text>
                ...
                {/* Settings List */}
                <View className="px-4 mt-8">
                    <Text className="text-lg font-bold text-[#111418] dark:text-white mb-3">Settings</Text>
                    <View className="bg-white dark:bg-[#1c2127] rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800">
...
const VitalCard = ({ icon, label, value }: any) => (
    <View className="bg-white dark:bg-[#1c2127] border border-slate-200 dark:border-slate-700 rounded-2xl p-4 mr-3 min-w-[130px]">
        <View className="flex-row items-center mb-2">
            {icon}
            <Text className="text-[#637588] dark:text-[#9dabb9] text-xs font-medium ml-2">{label}</Text>
        </View>
        <Text className="text-lg font-bold text-[#111418] dark:text-white">{value}</Text>
    </View>
);

const ListButton = ({ icon, label, sub, badge, indicator }: any) => (
    <TouchableOpacity className="flex-row items-center justify-between bg-white dark:bg-[#1c2127] p-4 rounded-2xl border border-slate-100 dark:border-transparent">
        <View className="flex-row items-center">
            <View className="h-10 w-10 rounded-full bg-slate-100 dark:bg-slate-800 items-center justify-center">
                {icon}
            </View>
            <View className="ml-3">
                <Text className="text-[#111418] dark:text-white font-semibold">{label}</Text>
                <Text className="text-[#637588] dark:text-[#9dabb9] text-[10px]">{sub}</Text>
            </View>
        </View>
        <View className="flex-row items-center">
            {badge && (
...
const ActionGridItem = ({ icon, label, emergency }: any) => (
    <TouchableOpacity
        className={`w-[48%] items-center justify-center p-5 mb-3 rounded-2xl border ${emergency
            ? 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-900/50'
            : 'bg-white dark:bg-[#1c2127] border-slate-200 dark:border-slate-800'
            }`}
    >
        {icon}
        <Text className={`text-sm font-medium mt-2 ${emergency ? 'text-red-700 dark:text-red-100' : 'text-slate-900 dark:text-white'}`}>
            {label}
        </Text>
    </TouchableOpacity>
);
...
const SettingsItem = ({ icon, label, last, danger, onPress }: any) => (
    <TouchableOpacity
        className={`flex-row items-center justify-between p-4 ${!last ? 'border-b border-slate-100 dark:border-slate-800' : ''}`}
        onPress={onPress}
    >
        <View className="flex-row items-center">
            {icon}
            <Text className={`ml-3 font-medium ${danger ? 'text-red-500' : 'text-slate-900 dark:text-white'}`}>{label}</Text>
        </View>
