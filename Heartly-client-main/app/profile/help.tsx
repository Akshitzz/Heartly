    ...
    return (
        <SafeAreaView className="flex-1 bg-[#f6f7f8] dark:bg-[#101922]">
            ...
            <View className="flex-row items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-slate-800 bg-white/90 dark:bg-[#101922]/90">
                ...
                <TouchableOpacity className="bg-red-500/10 px-3 py-1.5 rounded-full">
            ...
            <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} className="py-4 pl-4">
                    {['Top Questions', 'Appointments', 'Prescriptions', 'Insurance'].map((cat, i) => (
                        <TouchableOpacity
                            key={cat}
                            className={`mr-3 px-5 py-2 rounded-full border ${i === 0 ? 'bg-[#137fec] border-[#137fec]' : 'bg-white dark:bg-[#293039] border-gray-200 dark:border-transparent'
                                }`}
                        >
                            <Text className={`text-sm font-medium ${i === 0 ? 'text-white' : 'text-[#637588] dark:text-[#9dabb9]'}`}>
                                {cat}
                            </Text>
                        </TouchableOpacity>
                    ))}
                ...
                {/* FAQ List */}
                <View className="px-4">
                    <Text className="text-[#111418] dark:text-white text-xl font-bold pb-3 pt-4">Common Questions</Text>
                    {[
                        'How do I reschedule an appointment?',
                        'Where can I find my lab results?',
                        'How do I refill a prescription?',
                        'Is my university insurance accepted?',
                    ].map((q, i) => (
                        <TouchableOpacity key={i} className="flex-row items-center justify-between p-4 bg-white dark:bg-[#293039] rounded-xl mb-2 active:bg-gray-50 dark:active:bg-slate-700">
                            <Text className="text-[#111418] dark:text-white font-medium text-sm flex-1 mr-2">{q}</Text>
