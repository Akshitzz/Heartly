...
export class HospitalService {
    ...
    async getHospitalDoctors(hospitalId, params) {
        const query = { hospitalId, status: "active" };
        if (params.specialty) {
    ...
    async updateHospital(id, data) {
        const hospital = await Hospital.findByIdAndUpdate(id, { $set: data }, { new: true, runValidators: true });
        if (!hospital) {
            throw new AppError('Hospital not found', 404);
        }
    }
    ...
    async addDoctor(hospitalId, doctorId) {
        const doctor = await Doctor.findByIdAndUpdate(doctorId, { hospitalId }, { new: true });
        if (!doctor) {
            throw new AppError("Doctor not found ", 404);
        }
        return doctor;
    }
    async removeDoctor(hospitalId, doctorId) {
