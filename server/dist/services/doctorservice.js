...
export class DoctorService {
    ...
    async getDoctorReview(doctorId, page, limit) {
    ...
    async getAvailabiltiy(doctorId, date) {
    ...
    async updatedoctorProfile(doctorId, data) {
        const doctor = await Doctor.findByIdAndUpdate(doctorId, { $set: data }, { new: true, runValidators: true });
        if (!doctor) {
            throw new AppError("doctor not found ", 404);
        }
        // clear cache 
    ...
    async getSchedule(doctorId, params) {
    ...
    async VerifyDoctor(id) {
        const doctor = await Doctor.findByIdAndUpdate(id, { isVerified: true, status: "active" }, { new: true });
        if (!doctor) {
            throw new AppError('doctor not found ', 404);
        }
        return doctor;
