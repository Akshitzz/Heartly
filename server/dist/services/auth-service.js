...
export class AuthService {
    ...
    async register(data) {
        ...
        await this.redisClient.set(`refresh_token:${user._id.toString()}`, refreshToken, 7 * 24 * 60 * 60 // 7 days
        );
        return {
            user: user.toJSON(),
            tokens: {
    ...
    async login(data) {
        ...
        const refreshToken = JWTUtil.generateRefreshToken(user._id.toString());
        // Store refresh token
        await this.redisClient.set(`refresh_token:${user._id.toString()}`, refreshToken, 7 * 24 * 60 * 60);
    ...
    async logout(userId, refreshToken) {
        ...
        if (refreshToken) {
            await this.redisClient.set(`blacklisted_token:${refreshToken}`, 'true', 7 * 24 * 60 * 60);
        }
    }
